...
int main(int argc, char* argv[])
{
    ...
    sgm::cl::MaxDisparity disp_size =
        static_cast<sgm::cl::MaxDisparity>(parser.get<int>("max_disparity"));
    ...
    int input_depth = 8;
    params.subpixel = parser.get<bool>("subpixel") ? sgm::cl::DispPrecision::SUBPIXEL
                                                   : sgm::cl::DispPrecision::INTEGER;
    const int output_depth = (disp_size == sgm::cl::MaxDisparity::MAX_DISP_256 ||
                                 params.subpixel == sgm::cl::DispPrecision::SUBPIXEL)
                                 ? 16
                                 : 8;
    params.path_type = parser.get<int>("num_path") == 8 ? sgm::cl::PathType::SCAN_8PATH
                                                        : sgm::cl::PathType::SCAN_4PATH;
    params.uniqueness = 0.95f;

    {
        sgm::cl::StereoSGM ssgm(
            width, height, disp_size, sizeof(uint8_t) * 8, output_depth, cl_ctx, cl_device, params);
        ...
        d_disp = clCreateBuffer(
            cl_ctx, CL_MEM_READ_WRITE, width * height * output_depth / 8, nullptr, nullptr);
        ...
        while ((!should_close))
        {
            ...
            clEnqueueWriteBuffer(
                cl_queue, d_left, true, 0, width * height, left.data, 0, nullptr, nullptr);
            clEnqueueWriteBuffer(
                cl_queue, d_right, true, 0, width * height, right.data, 0, nullptr, nullptr);
            ...
            disp.convertTo(disparity_8u,
                CV_8U,
                255. / (static_cast<int32_t>(disp_size) *
                           (params.subpixel == sgm::cl::DispPrecision::SUBPIXEL ? 16 : 1)));
            ...
            cv::putText(disparity_color,
                "sgm execution time: " + std::to_string(dur.count()) + "[msec] " +
...
std::tuple<cl_context, cl_device_id> initCLCTX(int platform_idx, int device_idx)
{
    ...
    clGetDeviceIDs(
        platform_ids[platform_idx], CL_DEVICE_TYPE_GPU, num_devices, cl_devices.data(), nullptr);
    cl_device_id cl_device = cl_devices[device_idx];
    cl_int err;
    cl_context cl_ctx =
        clCreateContext(nullptr, 1, &cl_devices[device_idx], context_error_callback, NULL, &err);
    ...
    }
    {
        ...
        clGetPlatformInfo(
            platform_ids[platform_idx], CL_PLATFORM_NAME, 0, nullptr, &name_size_in_bytes);
        std::string platform_name;
        platform_name.resize(name_size_in_bytes);
        clGetPlatformInfo(platform_ids[platform_idx],
            CL_PLATFORM_NAME,
            platform_name.size(),
    ...
    }
    {
        ...
        clGetDeviceInfo(cl_devices[device_idx],
            CL_DEVICE_NAME,
            dev_name.size(),
            (void*)dev_name.data(),
            nullptr);
