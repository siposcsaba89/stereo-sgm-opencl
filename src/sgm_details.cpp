...
namespace cl
{
SGMDetails::SGMDetails(cl_context ctx, cl_device_id device, uint32_t input_bits)
    : m_cl_context(ctx)
    , m_cl_device_id(device)
{
    ...
    if (nullptr == m_kernel_check_consistency)
    {
        ...
        auto kernel_src =
            std::string(kernel_inttypes.begin(), kernel_inttypes.end()) +
        ...
        std::string kernel_SUBPIXEL_SHIFT =
            "#define SUBPIXEL_SHIFT " + std::to_string(SubpixelShift()) + "\n";
        kernel_src = std::regex_replace(kernel_src, std::regex("@SRC_T@"), kernel_template_types);
        kernel_src =
            std::regex_replace(kernel_src, std::regex("@SUBPIXEL_SHIFT@"), kernel_SUBPIXEL_SHIFT);

        m_program_check_consistency.init(m_cl_context, m_cl_device_id, kernel_src);
        m_kernel_check_consistency =
            m_program_check_consistency.getKernel("check_consistency_kernel");
    }
...
{
    cl_int err = clSetKernelArg(m_kernel_median, 0, sizeof(cl_mem), &d_src.data());
    ...
    size_t global_size[2] = {
        ((width + SIZE - 1) / SIZE) * local_size[0], ((height + SIZE - 1) / SIZE) * local_size[1]};

    err = clEnqueueNDRangeKernel(
        stream, m_kernel_median, 2, nullptr, global_size, local_size, 0, nullptr, nullptr);
    ...
    cl_command_queue stream)
{
    ...
    size_t global_size[2] = {
        ((width + SIZE - 1) / SIZE) * local_size[0], ((height + SIZE - 1) / SIZE) * local_size[1]};
    cl_int err = clSetKernelArg(m_kernel_check_consistency, 0, sizeof(cl_mem), &d_left_disp.data());
    err = clSetKernelArg(m_kernel_check_consistency, 1, sizeof(cl_mem), &d_right_disp.data());
    err = clSetKernelArg(m_kernel_check_consistency, 2, sizeof(cl_mem), &d_src_left);
    ...
    cl_command_queue stream)
{
    ...
    cl_int err = clSetKernelArg(m_kernel_disp_corr, 0, sizeof(cl_mem), &d_disp.data());
    ...
    err = clEnqueueNDRangeKernel(
        stream, m_kernel_disp_corr, 2, nullptr, global_size, local_size, 0, nullptr, nullptr);
    ...
    cl_command_queue stream)
{
    ...
    size_t global_size[1] = {((num_elements + SIZE - 1) / SIZE) * local_size[0]};
    ...
    err = clEnqueueNDRangeKernel(
        stream, m_kernel_cast_16uto8u, 1, nullptr, global_size, local_size, 0, nullptr, nullptr);
