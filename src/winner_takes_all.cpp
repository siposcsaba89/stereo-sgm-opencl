...
WinnerTakesAll::WinnerTakesAll(cl_context ctx,
    cl_device_id device,
    ...
    , m_right_buffer(ctx)
{
    ...
    std::string kernel_max_disparoty =
        "#define MAX_DISPARITY " + std::to_string(static_cast<uint32_t>(max_disparity)) + "\n";
    int NUM_PATHS = path_type == PathType::SCAN_4PATH ? 4 : 8;
    std::string kernel_NUM_PATHS = "#define NUM_PATHS " + std::to_string(NUM_PATHS) + "\n";
    std::string kernel_COMPUTE_SUBPIXEL =
        "#define COMPUTE_SUBPIXEL " + std::to_string(precision == DispPrecision::SUBPIXEL ? 1 : 0) +
        "\n";
    std::string kernel_WARPS_PER_BLOCK =
        "#define WARPS_PER_BLOCK " + std::to_string(WARPS_PER_BLOCK) + "\n";
    ...
    kernel_src =
        std::regex_replace(kernel_src, std::regex("@COMPUTE_SUBPIXEL@"), kernel_COMPUTE_SUBPIXEL);
    kernel_src =
        std::regex_replace(kernel_src, std::regex("@WARPS_PER_BLOCK@"), kernel_WARPS_PER_BLOCK);
...
void WinnerTakesAll::enqueue(const DeviceBuffer<uint8_t>& src,
    int width,
    int height,
    int pitch,
    float uniqueness,
...
void WinnerTakesAll::enqueue(DeviceBuffer<uint16_t>& left,
